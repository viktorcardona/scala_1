 ...
 */
 @RunWith(classOf[JUnitRunner])
   ...
   */
  test("details why one plus one is not three, but now it is fixed.") {
  ...
  test("max of a few numbers - 2") {
    assert(max(List(3, 7, 2, -7, 9, 0, 11, -1, 73, 55, -77)) === 73)
  }

  test("max of a few numbers - 3 - intercepting java.util.NoSuchElementException.") {
  ...
  test("sum of a few numbers - 2") {
    assert(sum(List(1,2,0,0,0,0,0,0,0,7,0,0,0,0,0,0,0,0,0)) === 10)
  }
